...
if (isServer and isNil "sm_done") then {
...

	{
		...
		if ((_class isKindOf "AllVehicles") && ((_CharacterID == "0") OR (_CharacterID == "1")) && (_damage < 1)) then {
			...
			//Limit ammo of UH1Y
			if (_entity isKindOf "UH1Y_DZ") then {
			_entity setVehicleAmmo 0.2;
			};
				
			//Limit Ammo of ArmoredSUV_PMC
			if (_entity isKindOf "ArmoredSUV_PMC") then {
			_entity setVehicleAmmo 0.2;
			};
				
			//Limit Ammo of GPK
			if (_entity isKindOf "HMMWV_M1151_M2_DES_EP1") then {
			_entity setVehicleAmmo 0.5;
			};

			//Limit Ammo of UH60M_DZ
			if (_entity isKindOf "UH60M_DZ") then {
			_entity setVehicleAmmo 0.2;
			};
			
			//Limit Ammo of Camel_DZ
			if (_entity isKindOf "Camel_DZ") then {
			_entity setVehicleAmmo 0.3;
			};
