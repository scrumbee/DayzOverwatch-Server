...
while {_locking} do {
	...
	if(client_actionInterrupt || isNull(_object) || (player distance _object > 50)) exitWith {
		...
		player globalChat "Locking cancelled!";

		_failed = true;
	};

	if((_loops % 10) == 0) then {
		cutText [format["Locking object... (%1)", ceil((_loops_trgt - _loops) / 10)], "PLAIN DOWN", 1];
	};
...
if(!_failed) then {
	(_this select 0) setVariable ["R3F_Locked", true, true];

	R3F_LOG_mutex_local_verrou = false;

	cutText ["", "PLAIN DOWN", 0];
	player globalChat "Object locked!";
};
